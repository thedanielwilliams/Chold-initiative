export default async function handler(req, res) {
  ...
  const { name, email, phone, location, area, type, cv, message } = req.body || {};
  ...
  try {
    ...
    if (!appRes.ok) {
      console.error('Resend Application Email Error:', appData);
      return res.status(appRes.status).json({ error: appData.message || 'Failed to send application response.' });
    }
